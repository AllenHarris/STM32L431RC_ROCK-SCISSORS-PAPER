...
void gpio_pull(uint_16 port_pin, uint_8 pullselect)
{
	...
	//设定为高速输出状态
	gpio_ptr->OSPEEDR |= (uint_32)(HIGH_SPEED << (pin * 2u));

	if(1 == pullselect)
		//引脚拉高电平
		gpio_ptr->PUPDR |= (uint_32)(PULL_UP << (pin * 2u));
	else
		//引脚拉低电平
		gpio_ptr->PUPDR |= (uint_32)(PULL_DOWN << (pin * 2u));
}
...
//	uint_32 temp;       //临时存放寄存器里的值
	...
	//配置引脚的驱动能力
	gpio_ptr->OSPEEDR |= (uint_32)(control << (pin * 2u));
}
